...
/** True when the build will actually fall back to the SDK debug keystore. */
val usingDebugReleaseSigning = releaseSigningIssue != null && allowDebugReleaseSigning
...
android {
    ...
    buildTypes {
        release {
            ...
            // A release signed with the SDK's debug keystore is not distributable:
            // that keystore is regenerated by the SDK, which already broke the
            // upgrade lineage once. Stamp the versionName so such an artifact is
            // identifiable in About, `adb shell dumpsys package`, and bug reports
            // instead of being indistinguishable from a properly signed build.
            if (usingDebugReleaseSigning) {
